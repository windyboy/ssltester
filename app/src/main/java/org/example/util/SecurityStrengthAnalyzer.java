...
public class SecurityStrengthAnalyzer {
    ...
    public static String analyzeProtocol(String protocolVersion) {
        ...
        String upperProtocol = protocolVersion.toUpperCase();
        for (String weak : WEAK_PROTOCOLS) {
            if (upperProtocol.contains(weak)) {
                return STRENGTH_WEAK;
            }
        }
        for (String strong : STRONG_PROTOCOLS) {
            if (upperProtocol.contains(strong)) {
                return STRENGTH_STRONG;
            }
        }
        return STRENGTH_ADEQUATE; // Default for unrecognized but not explicitly weak (e.g. future versions)
    }
