...
class CertificateValidatorTest {
    ...
    fun `test validateCertificateChain with single certificate`() {
        val certificate = createTestCertificate(
    ...
    fun `test certificate strength classification`() {
        val weakCert = createTestCertificate(
            subject = "CN=example.com",
            notAfter = Instant.now().plus(30, ChronoUnit.DAYS),
            keySize = 1024
        )
        
        val mediumCert = createTestCertificate(
            subject = "CN=example.com",
            notAfter = Instant.now().plus(30, ChronoUnit.DAYS),
            keySize = 2048
        )
        
        val strongCert = createTestCertificate(
            subject = "CN=example.com",
            notAfter = Instant.now().plus(30, ChronoUnit.DAYS),
            keySize = 4096
        )
    ...
    fun `test certificate expiry edge cases`() {
        val expiringToday = createTestCertificate(
            subject = "CN=example.com",
            notAfter = Instant.now().plus(1, ChronoUnit.HOURS),
            keySize = 2048
        )
        
        val expiringTomorrow = createTestCertificate(
            subject = "CN=example.com",
            notAfter = Instant.now().plus(25, ChronoUnit.HOURS),
            keySize = 2048
        )
        
        val expiringNextMonth = createTestCertificate(
            subject = "CN=example.com",
