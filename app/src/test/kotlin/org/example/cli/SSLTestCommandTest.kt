...
class SSLTestCommandTest {
    ...
    fun setUp() {
        ...
        testLocator = TestServiceLocator(
    ...
    @Test
    fun `test command with valid parameters`() = runBlocking {
        val command = SSLTestCommand()
        ...
        // 模拟成功的SSL连接
        val mockConnection = SSLConnection(
            host = "example.com",
            port = 443,
            protocol = "TLSv1.3",
    ...
    @Test
    fun `test command with connection failure`() = runBlocking {
        val command = SSLTestCommand()
